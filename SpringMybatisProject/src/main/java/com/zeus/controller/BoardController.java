...
public class BoardController {
	...
	//게시판입력내용저장하고 성공메세지를 (/WEB-INF/views/board/succeess.jsp)
	...
	//게시글상세내용글 요청 -> 한개게시판정보를 가져와서 -> (/WEB-INF/views/board/read.jsp) 화면에 전달한다.
	...
	//게시글수정하기 위한 화면요청 -> 해당된 게시글을 가져와서 -> (/WEB-INF/views/board/modify.jsp) 화면에 전달한다.
	@RequestMapping(value = "/modify", method = RequestMethod.GET)
	public void modifyForm(int boardNo, Model model) throws Exception {
		model.addAttribute(service.read(boardNo));
	}

	//게시글수정내용을 디비저장요청 -> 수정된 게시글 수정완료 -> 결과화면 성공메세지를 (/WEB-INF/views/board/success.jsp)
